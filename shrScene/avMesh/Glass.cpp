...
Texture2D Back; SamplerState BackSampler;
float BackOffsetStep;
...
PS_Output PS(VS_Output In) {
    ...
    float3 offset = Maps.Sample(MapsSampler, In.vTex).xyz;
    float4 bck = Back.Load(int3(In.Pos.xy+offset.xz*BackOffsetStep/In.Pos.w, 0));    
    
    float4 diff = lerp(bck, 1.0, offset.y);  
    Out.Color = diff;
